...
class QueryBaseTest: StringSpec() { init {
    ...
    "out parameter" {
        ...
        """.trimIndent().let { QueryBase(it) }

        println(query)

        assertEquals(1, query.paramStructs.size)

        val param = query.paramStructs.first()
        assertEquals("seq", param.key)
        assertEquals(JdbcType.NUMERIC, param.jdbcType)
        assertEquals(true, param.out)
